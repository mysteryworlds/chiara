...
public class PermissionsCommand implements CommandExecutor, TabCompleter {
  ...
  private boolean showUserGroups(CommandSender commandSender, Command command, String playerName) {
    ...
    if (groups.isEmpty()) {
      commandSender.sendMessage(String
          .format("%sIt seems like %s%s%s isn't member of any group.", MESSAGE_PREFIX,
              ChatColor.GOLD, playerName, ChatColor.YELLOW));
    } else {
      commandSender.sendMessage(String
          .format("%s%s%s%s is member %s%d%s groups:", MESSAGE_PREFIX, ChatColor.GOLD, playerName,
              ChatColor.YELLOW, ChatColor.GOLD, groups.size(), ChatColor.YELLOW));
   ...
   * @return If its a valid command.
   */
  private boolean removeUserGroup(CommandSender commandSender, Command command, String playerName,
      String groupName) {
    ...
    commandSender.sendMessage(String
        .format("%sDu hast %s%s%s aus der Gruppe Gruppe %s%s%s entfernt.", MESSAGE_PREFIX,
            ChatColor.GOLD, playerName, ChatColor.YELLOW, ChatColor.GOLD, groupName,
            ChatColor.YELLOW));
    return true;
  }
   ...
   * @return If its a valid command.
   */
  private boolean addUserGroup(CommandSender commandSender, Command command, String playerName,
      String groupName) {
    ...
    commandSender.sendMessage(String
        .format("%You have added %s%s%s to group %s%s%s.", MESSAGE_PREFIX, ChatColor.GOLD,
            playerName, ChatColor.YELLOW, ChatColor.GOLD, groupName, ChatColor.YELLOW));
    return true;
  }
